...
// See page 248 in
// "Assembly Language Programming for the Color Computer (1985)(Laurence A
// Tepolt)(pdf)". Our TextMode is Display Mode "AI". Our PMode1 is Display Mode
// "G3C".
//
...
void NowSwitchToGameScreen() {
  if (0) PutChar('V');
  NowSwitchDisplayMode(Vdg.game_framebuffer, Vdg.game_vdg_mode,
                       Vdg.game_sam_mode);
}
...
void NowSwitchToChatScreen() {
  if (0) PutChar('U');
  NowSwitchToDisplayText(Cons,
                         gKern.in_game ? COLORSET_ORANGE : COLORSET_GREEN);
}

void Vdg_Init() { NowSwitchToChatScreen(); }
